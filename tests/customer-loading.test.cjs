...
function harness(fetchImpl, initialToken) {
  ...
  function element() {
    return { children: [], attributes: { placeholder: 'Search customers' }, placeholder: 'Search customers',
  ...
  const context = {
    window: { __authToken: initialToken }, A: 'anonymous-key', SUPA: 'https://example.supabase.co',
